import{_ as i,o as a,c as n,a3 as p}from"./chunks/framework.DemfeeZg.js";const g=JSON.parse('{"title":"QQ 私有化部署文档","description":"","frontmatter":{},"headers":[],"relativePath":"QQ_API.md","filePath":"QQ_API.md","lastUpdated":1789921234000}'),l={name:"QQ_API.md"};function h(k,s,t,e,r,d){return a(),n("div",null,[...s[0]||(s[0]=[p(`<h1 id="qq-私有化部署文档" tabindex="-1">QQ 私有化部署文档 <a class="header-anchor" href="#qq-私有化部署文档" aria-label="Permalink to &quot;QQ 私有化部署文档&quot;">​</a></h1><h2 id="_1-下载-go-cq" tabindex="-1">1. 下载 go-cq <a class="header-anchor" href="#_1-下载-go-cq" aria-label="Permalink to &quot;1. 下载 go-cq&quot;">​</a></h2><p>前往<a href="https://github.com/Mrs4s/go-cqhttp" target="_blank" rel="noreferrer">go-cqhttp release</a>下载对应系统版本。</p><h2 id="_2-配置服务" tabindex="-1">2. 配置服务 <a class="header-anchor" href="#_2-配置服务" aria-label="Permalink to &quot;2. 配置服务&quot;">​</a></h2><p>解压</p><blockquote><p>Windows 下请使用自己熟悉的解压软件自行解压</p></blockquote><blockquote><p>Linux 下在命令行中输入 tar -xzvf [文件名] 使用</p></blockquote><p><strong>Windows 标准方法</strong></p><p>双击，根据提示生成运行脚本 go-cqhttp_*.exe</p><p><code>[WARNING]: 尝试加载配置文件 config.yml 失败：文件不存在 [INFO]: 默认配置文件已生成，请编辑 config.yml 后重启程序.</code></p><p>配置文件请参考下方 config.yml</p><p>config.yml 配置好后 再次双击运行脚本</p><p><code>[INFO]: 登录成功 欢迎使用: balabala</code></p><p>如出现需要认证的信息，请自行认证设备。</p><p>此时，基础配置完成</p><p><strong>Linux 标准方法</strong></p><p>通过 SSH 连接到服务器</p><p>cd 到解压目录</p><p>输入 , 运行 <code>./go-cqhttp</code></p><p><code>[WARNING]: 尝试加载配置文件 config.yml 失败：文件不存在 [INFO]: 默认配置文件已生成，请编辑 config.yml 后重启程序.</code></p><p><strong>配置 config.yml</strong></p><div class="language-yaml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">yaml</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"># go-cqhttp 默认配置文件</span></span>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">  uin</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">: </span><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"># QQ 账号</span></span>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">    max-times</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">: </span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">0</span><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"> # 最大重连次数，0 为无限制</span></span>
<span class="line"></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">  # 是否忽略无效的 CQ 码，如果为假将原样发送</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">  # 是否将 url 分片发送</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">  # 移除服务端的 Reply 附带的 At</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">  # 为 Reply 附加更多信息</span></span>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">default-middlewares</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">: </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">&amp;</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">default</span><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"> # 访问密钥，强烈推荐在公网的服务器设置</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">  # API 限速设置</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    # 是否启用内置 leveldb 数据库</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">    # 启用将会增加 10-20MB 的内存占用和一定的磁盘空间</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">      # 反向 HTTP 超时时间，单位秒</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">      # 最小值为 5，小于 5 将会忽略本项设置</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">      # 反向 HTTP POST 地址列表</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">      #  secret: &#39;&#39;          # 密钥</span></span></code></pre></div><p>再次运行<code>./go-cqhttp</code></p><p><code>[INFO]: 登录成功 欢迎使用: balabala</code></p><p>如出现需要认证的信息，请自行认证设备。</p><p>此时，基础配置完成</p><h2 id="注意-将你配置的端口号在防火墙里面放行或者使用反向代理将你设置的端口绑定到域名" tabindex="-1">注意：将你配置的端口号在防火墙里面放行或者使用反向代理将你设置的端口绑定到域名 <a class="header-anchor" href="#注意-将你配置的端口号在防火墙里面放行或者使用反向代理将你设置的端口绑定到域名" aria-label="Permalink to &quot;注意：将你配置的端口号在防火墙里面放行或者使用反向代理将你设置的端口绑定到域名&quot;">​</a></h2><h2 id="注意-公网使用一定要配置-token" tabindex="-1">注意：公网使用一定要配置 token <a class="header-anchor" href="#注意-公网使用一定要配置-token" aria-label="Permalink to &quot;注意：公网使用一定要配置 token&quot;">​</a></h2><h2 id="_3-twikoo-配置" tabindex="-1">3. twikoo 配置 <a class="header-anchor" href="#_3-twikoo-配置" aria-label="Permalink to &quot;3. twikoo 配置&quot;">​</a></h2><p>在 twikoo 中 QQ 私有化 API 配置项填写如下内容</p><p>QQ 号 <code>http://你的IP地址：端口号（或者域名）/send_private_msg?user_id=QQ号?token=你配置的token</code></p><p>QQ 群 <code>http://你的IP地址：端口号（或者域名）/send_group_msg?token=你配置的token?group_id=群号</code></p><p>配置完成</p>`,33)])])}const y=i(l,[["render",h]]);export{g as __pageData,y as default};
