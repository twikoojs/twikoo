...
| ---- | ---- | ---- |
| [腾讯云 CloudBase 部署](#腾讯云-cloudbase-部署) | ★★★☆☆ | 手动部署到腾讯云云开发环境，在中国大陆访问速度较快。**需要标准版及以上套餐**：免费体验版与个人版的云函数执行超时固定 3 秒且不可修改，发评论会在评论入库前被中断。 |
...
::: warning 前提：需要标准版及以上套餐
免费体验版与个人版的云函数**执行超时固定 3 秒、内存固定 256MB，均不可修改**（控制台上该表单项为禁用状态并带「个人版」标记）。
Twikoo 发评论在冷实例上会超过 3 秒，函数被强制中断、评论不会入库（实测：读评论正常，发评论返回 `Invoking task timed out after 3 seconds`）。
标准版及以上可将执行超时上调至 900 秒。
:::
