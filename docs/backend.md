...
| [腾讯云命令行部署](#腾讯云命令行部署)               | ⛔     | **2.0 起不再支持**（BC-14）：仅影响部署操作，函数功能不受影响；请改用控制台部署。                                                                                                                              |
...
| 能力        | 含义              | 全能力适配器 | EdgeOne Makers                                                              |
| ----------- | ----------------- | ------------ | --------------------------------------------------------------------------- |
| mail        | 邮件通知          | ✅           | ⚠️ **受限**：仅 SendGrid / MailChannels，或经 `SMTP_HOST` 走 Go SMTP Bridge |
| domPurify   | 评论内容 XSS 消毒 | ✅           | ❌ 直通（内容原样存储，请自行评估）                                         |
| ip2region   | IP 属地展示       | ✅           | ✅                                                                          |
| akismet     | Akismet 反垃圾    | ✅           | ❌                                                                          |
| tencentTms  | 腾讯云文本安全    | ✅           | ❌                                                                          |
| imageUpload | 图片上传          | ✅           | ✅                                                                          |
...
> 受限能力不影响评论的收发与展示：缺少的能力在管理面板对应配置项上会返回明确的错误提示，而不是静默失败。

::: warning 运行时版本
CloudBase 请将云函数运行时升级到 **Node 20 及以上（推荐 24）**；2.0 产物语法目标为 ES2022，不再兼容 Node 16.13（BC-3）。
:::
...
::: danger 2.0 起不再支持
Twikoo **2.0 起移除了命令行部署**（`tcb fn deploy` 与配套的 `npm run deploy` / `login` / `logout` 脚本、`@cloudbase/cli` 依赖一并删除，BC-14）。

- **函数功能不受影响**，也不影响控制台部署方式；
- 若您此前使用 `tcb fn deploy` 或自定义 CI 脚本部署，请改用上方[腾讯云手动部署](#腾讯云手动部署)（控制台「在线编辑器 + 在线装依赖」，实测可用）；
- 本节内容仅为 1.x 历史流程留档，**不再适用于 2.0**。
  :::
